...
String _prepareXhtml(
  ...
  try {
    ...
    final head = root.childElements
        ...
        .firstOrNull;
    // Mobile WebViews lay out pages without a viewport declaration at a
    // desktop width; fixed layouts keep the author's viewport.
    if (!fixedLayout) {
      (head ?? root).children.add(
        XmlElement.tag(
          ...
          namespaceUri: xhtmlNamespace,
          attributes: [
            XmlAttribute(XmlName.parts('name'), 'viewport'),
            XmlAttribute(XmlName.parts('content'), _viewport),
          ],
        ),
      );
    }
...
/// The viewport declared for reflowable documents.
