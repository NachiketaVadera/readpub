...
(() => {
  ...
  // Right-to-left scroll containers report negative scrollLeft values.
  const direction = () => (getComputedStyle(document.body).direction === 'rtl' ? -1 : 1);
  ...
  function scrollToProgression(value) {
    ...
    if (paged()) {
      ...
      body.scrollLeft = direction() * Math.round(target / window.innerWidth) * window.innerWidth;
    } else {
      const element = document.scrollingElement || root();
